...
template <typename T> class Abb {
    public:
        ...
        void eliminateSubtree(const T &e); //Exercise1 of BINARY SEARCH TREE
        ~Abb(); // destructor
    private:
        struct arbol {
            T elto;
        ...
        T borrarMin();
        void eliminateSub(Abb<T> &abb, const T &e);
};
...
template <typename T>
void Abb<T>::eliminateSubtree(const T &e){
    eliminateSub(r,e);
}
...
void eliminateSub(Abb<T> &abb, const T &e){
    if(abb != nullptr){
        if(e < abb->elemento) eliminateSub(abb->izqdo.r, e);
        else if(e > abb->elemento) eliminateSub(abb->drcho.r, e);
        else{
            delete abb; 
            abb = nullptr;
        }
...
/*
template <typename T>
void eliminateSub(arbol* &abb, const T &e){
    if(abb != nullptr){
        if(e < abb->elemento) eliminateSub(abb->izq.r, e);
        else if(e > abb->elemento) eliminateSub(abb->drc.r, e);
        else{
            delete abb; 
            abb = nullptr;
        }
