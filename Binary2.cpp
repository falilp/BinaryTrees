...
/* exercise 3:
The TAD binary tree can accommodate mathematical expressions using an expression tree. Within the binary tree the leaf nodes will contain the operands, and the rest
...
// RENEGADES: A node is said to be renegade when it goes against the others. Given a Binary Tree, calculate the number of renegade nodes in said tree.
